...
model.head = nn.Linear(model.head.in_features, len(charDef['charList']))
"""
# The ViT model
pretrained_model = 'vit_base_patch16_224'
model = timm.create_model(pretrained_model, pretrained=True)
...
model.head = nn.Linear(original_head.in_features, len(charDef['charList']))
...
# Save Model
torch.save(model.state_dict(), './vit_single_chars_1.pth')
model.load_state_dict(torch.load('./vit_single_chars_1.pth'))
